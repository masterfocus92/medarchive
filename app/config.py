...
class Settings(BaseSettings):
    ...
    # Среда исполнения (ADR-015). stg показывает плашку «Тестовый стенд»:
    # данные стенда перезаписываются синком prod2stg каждую ночь (ADR-016),
    # запись, внесённая туда по ошибке, погибнет — среда обязана быть видна.
    app_env: Literal["dev", "stg", "prod"] = "dev"
