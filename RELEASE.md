...
5. На GitHub: **Pull requests → New** → base `main` ← compare `stg` → Create → дождаться зелёного чека `tests` → **Merge**. (Прямой push в `main` запрещён protection'ом — прод пополняется только PR'ом из `stg`.)
6. Actions → `deploy-prod` → **Review deployments** → Approve → прод обновляется.
7. Смоук прода: вход, лента, открыть одну запись.
