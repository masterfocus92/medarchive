...
## 6. Схема: жизненный цикл записи (статусы)

```plantuml
@startuml
title Статусы записи (видны в UI текстом, без спиннеров-заглушек)

[*] --> Загружено : POST /records\nфайл на диске, запись в БД
[*] --> Подтверждена : запись без файла (только заметка) ❓\nконвейер пропущен

Загружено --> Разбирается : фон-задача взяла в работу (Э4)
Разбирается --> Разобрано : Claude вернул поля\n(черновик, ничего не перезаписывает молча)
Разбирается --> Разбор_не_удался : ошибка / таймаут AI\n(сохранение не отменяется — инвариант)

Разобрано --> Подтверждена : «Сохранить» на экране проверки\n(правки приняты человеком)
Разбор_не_удался --> Подтверждена : ручное заполнение + «Сохранить»

Подтверждена : Э7: генерируются эмбеддинги,\nзапись попадает в поиск
Разобрано : в ленте видна, в поиске — нет
Разбор_не_удался : в ленте видна, в поиске — нет
@enduml
```
