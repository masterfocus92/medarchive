...
>
> **Обновление 15.07.2026 — кит v2 (этап 5.5) — утверждено владельцем 15.07.2026.** Поведение, маршруты и статусная модель не меняются; меняется визуальный слой:
> первичное действие каждого экрана переезжает в нижний бар `action-bar` (зона большого пальца);
> B4 получает видимое воплощение — `btn-primary:disabled` + helper «Добавьте фото или заметку» как **JS-зеркало** инварианта (без JS кнопка активна, инвариант держит сервер той же ошибкой — ADR-011);
> превью-список страниц — примитивы `pages`/`page-thumb`/`page-add` кита v2 («＋ ещё страница» заложена в примитив);
> статус на экране проверки — бейджем `badge` (текстом); AI-заполненные поля помечены `field.ai` + `ai-hint`;
> предложение AI-пациента — баннер `ai-suggest` **без кнопки «Принять»**: принятие — тап по пунктирной монограмме (решение 15.07.2026);
> превью рендерит клиентский JS-накопитель (`record-form.js`): **серверного staging страниц нет**, файлы впервые уходят на сервер одним `POST /records`, htmx не вводится (решение 15.07.2026, ADR-011 в силе); выбор пациента — монограммы, не чипы.
...
| **Добавление записи** | **Два входа для контента** (см. §4а): **«Сфотографировать»** — `input capture="environment"`, открывает камеру, по одному кадру, цикл «＋ Ещё страница» · **«Выбрать из галереи или файл»** — `input multiple` (фото/PDF), мультивыбор одним заходом · общий превью-список страниц — `pages`/`page-thumb`/`page-add` кита v2 (порядок значим, страница удаляется до сохранения, «＋ ещё» в примитиве) · заметка `textarea.pencil` (карандаш) · пациент — выбор из членов семьи (монограммы/чипы), **дефолт — активный профиль** · **«Сохранить»** `btn-primary` в нижнем баре `action-bar` (+ disabled/helper при пустой форме — JS-зеркало B4) · «Отмена» рядом в баре | Э3, бар — Э5.5 |
| **Экран проверки** (он же режим правки, Э6) | Статус — бейдж `badge` (текстом, вариант по состоянию; всегда виден) · поля `field`: название, дата события (`control.data`, mono), клиника, врач, тип (`chip`, открытое), содержание — **AI-заполненные несут `field.ai` + `ai-hint` «подставил AI · проверьте»** · заметка-карандаш · пациент с подсветкой AI-предложения · **«Сохранить»** `btn-primary` в нижнем баре `action-bar` | Э4, кит v2 — Э5.5 |
...
- **B4. Ни файла, ни заметки** — «Сохранить» недоступна (`btn-primary:disabled` кита v2) + helper «Добавьте фото или заметку» в `action-bar` (инвариант: пустых записей нет). Недоступность — JS-зеркало: без JS кнопка активна, сервер отвечает той же ошибкой (ADR-011). Из тупика — добавить контент или «Отмена».
...
- **B7. AI предложил другого пациента** — на проверке предложение подсвечено и помечено `ai-hint`; принятие — один тап по монограмме (отдельной кнопки «Принять» нет — решение 15.07.2026); выбранный при создании не затирается молча.
...
> **Статус:** ❓1–3 и ❓7–10 зафиксированы нарезкой этапа 3 (`tasks/TASK-3.md`). ❓4–6 зафиксированы нарезкой этапа 4 (`tasks/TASK-4.md`, шапка): meta-refresh вместо поллинга, BackgroundTasks, ретрай включён в этап.
