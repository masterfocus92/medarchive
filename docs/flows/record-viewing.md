...
|---|---|---|
| **Лента** (главная, вместо счётчика) | Шапка `app-head` (есть) · «Добавить запись» `btn-primary btn-lg` (есть) · список элементов-записей сверху вниз, новые первыми: композиция `record` в компактном виде — `rec-head` (название serif + дата mono), `rec-meta` (тип `chip`, клиника), **статус-бейдж на неподтверждённых** · пустое состояние (есть) | Э5 |
...
1. **Сортировка ленты — по дате внесения** (`created_at` desc, новые сверху). Дата события показывается в элементе, но не сортирует: лента — хроника внесения, «что я вносил недавно» (фильтры/сортировки — Could, бэклог). Альтернатива — по `event_date` — путает: свежевнесённая старая справка утонет.
